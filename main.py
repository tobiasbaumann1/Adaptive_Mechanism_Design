...
if __name__ == "__main__":
    ...
    actor_a_prob_each_round = np.transpose(np.array([agent.get_actions_prob_log() for agent in agents]))
    ...
    policing_a_prob_each_round = np.transpose(np.array([policing_agent.get_actions_prob_log() for agent in agents]))
    print(policing_a_prob_each_round.shape)
    plot_results(policing_a_prob_each_round,[str(agent) for agent in agents],env.__str__()+'_policing_action_probabilities', ylabel = 'P(a_p=1)')
